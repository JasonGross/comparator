...
structure State where
  worklist : Array Lean.Name
  checked : Std.HashSet Lean.Name
  forbiddenAxioms : Array Lean.Name
...
partial def loop : GradeM Unit := do
  ...
  if (← get).checked.contains target then
    ...
    if let .axiomInfo info := info then
      if !(← read).legalAxioms.contains info.name then
        modify fun s => { s with forbiddenAxioms := s.forbiddenAxioms.push target }

    if (← read).targetNames.contains target then
      if info.isUnsafe then
        modify fun s => { s with forbiddenAxioms := s.forbiddenAxioms.push target }
      if info.isPartial && !(← read).allowPartial then
        modify fun s => { s with forbiddenAxioms := s.forbiddenAxioms.push target }
...
def checkAxiomsGrade (solution : ExportedEnv) (targets : Array Lean.Name) (legal : Array Lean.Name) (allowPartial : Bool) :
    Except String (Array Lean.Name) := do
  ...
  let prog := do
    targets.forM Axioms.Grade.addWorklist
    Axioms.Grade.loop
  let (_, state) ← prog.run { solution, allowPartial, legalAxioms, targetNames } |>.run { worklist := #[], checked := {}, forbiddenAxioms := #[] }
  return state.forbiddenAxioms
